...
def create_tag(client: TestClient, index: int = 1) -> dict:
    response = client.post("/api/v1/tags/", json=make_tag_payload(index))
...
def test_create_tag_without_slug_uses_name(client: TestClient):
    payload = make_tag_payload()
    payload.pop("slug")
...
def test_get_tag_by_slug(client: TestClient):
    tag = create_tag(client, 1)
...
def test_get_tag_with_posts(client: TestClient):
    user = create_user(client, 1)
    tag = create_tag(client, 1)
    post = create_post_for_tag(client, user["id"], tag["id"], 1)
...
def test_get_tag_with_posts_by_slug(client: TestClient):
    user = create_user(client, 1)
    tag = create_tag(client, 1)
    post = create_post_for_tag(client, user["id"], tag["id"], 1)
...
def test_update_tag(client: TestClient):
    ...
    response = client.patch(f"/api/v1/tags/{tag['id']}", json=payload)
